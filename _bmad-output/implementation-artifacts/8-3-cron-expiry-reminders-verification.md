...
- 2026-07-07 (BMAD code review follow-up) — Correctifs appliqués : borne paiement couvrant passée en semi-ouverte (`periodEnd > subscriptionEnd`), rappels email protégés contre les exécutions concurrentes par claim-before-send + suppression du claim si `sendEmail` échoue, `enforceTenantAccess` réaligné sur la règle produit lecture-seule pour suspension non-paiement et redirect uniquement pour `totalBlock`/`cancelled`. Gaps encore ouverts : Vercel Cron production + `CRON_SECRET` dashboard non vérifiés ; livraison réelle depuis le chemin cron non prouvée sans `RESEND_API_KEY` local.
