...
AND    les ops avec failed = true ne sont PAS rejoués automatiquement — nécessitent action manuelle
```
...
- [ ] Remplacer la requête liveQuery défectueuse :
  ```typescript
...
- [ ] Vérifier que `pendingCount` s'incrémente correctement lors d'ajout dans syncQueue
- [ ] `pnpm typecheck` — zéro erreur

### T2 — Ajouter `registerBackgroundSync()` dans `src/lib/sync/outbox.ts`

- [ ] Ajouter la fonction d'enregistrement Background Sync (feature-detected, silencieuse si non supporté) :
  ```typescript
...
- [ ] Appeler `registerBackgroundSync()` à la fin de `applyLocalMutation()` (après la transaction Dexie) :
  ```typescript
...
- [ ] Ajouter après `serwist.addEventListeners()` :
  ```typescript
  ...
  self.addEventListener("sync", (event: Event) => {
    ...
    if (syncEvent.tag === "quotation-sync") {
      syncEvent.waitUntil(
        (async () => {
          try {
            // Import dynamique pour éviter le chargement Dexie dans le SW initial
            // Note : le SW n'a pas accès direct à db — il doit passer par fetch
            // Pattern : fetch vers /api/v1/sync/push avec les ops en attente
            // OU : utilisation de Dexie dans le SW (si supporté dans ce contexte)
...
- [ ] Implémenter `syncFromServiceWorker()` dans le SW :
  ```typescript
  // Le SW n'a pas accès au module outbox (client-only).
  // Il doit soit : (1) accéder à Dexie directement (avec import), soit
  // (2) envoyer un message à tous les clients actifs pour déclencher triggerSync.
  // OPTION RECOMMANDÉE pour MVP-1 : messagerie SW → client actif (plus simple, évite Dexie dans SW)
  async function syncFromServiceWorker(): Promise<void> {
    ...
    if (clients.length > 0) {
      ...
      // via fetch /api/v1/sync/push (nécessite session valide — cookie envoyé automatiquement)
      // Si non authentifié, la sync échouera avec 401 — système normal, pas de retry
      await directSyncFromSW();
    }
  }
  ```

- [ ] Ajouter le type `SyncEvent` (non standard dans TypeScript par défaut) :
  ```typescript
...
- [ ] `pnpm typecheck` — zéro erreur sur sw.ts
...
- [ ] Ajouter dans `useSyncStatus()` un listener sur les messages SW :
  ```typescript
  useEffect(() => {
    if (!("serviceWorker" in navigator)) return;
...
- [ ] S'assurer que le cleanup `removeEventListener` est en place
- [ ] `pnpm typecheck` — zéro erreur
...
- [ ] Implémenter dans `sw.ts` :
  ```typescript
  async function directSyncFromSW(): Promise<void> {
    // Lire la queue Dexie depuis le SW (Dexie fonctionne dans SW en tant que IndexedDB client)
    // Importer Dexie et la classe LocalDatabase dans le SW
    // Note : "use client" sur local-db.ts est une directive Next.js, pas native —
    //        le SW peut importer Dexie directement sans ce directive
    const { db } = await import("@/lib/local-db");
    const pendingOps = await db.syncQueue.filter((op) => !op.failed).sortBy("queuedAt");
    if (pendingOps.length === 0) return;

    // Push FIFO via fetch (cookie de session envoyé automatiquement)
    const res = await fetch("/api/v1/sync/push", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ops: pendingOps.slice(0, 10) }), // batch 10 max pour le SW
    });

    if (res.ok) {
      // Supprimer les ops appliquées de la queue
      const result = await res.json() as { results: Array<{ opId: string; status: string }> };
      for (const r of result.results) {
        if (r.status !== "failed") {
          await db.syncQueue.delete(r.opId);
        }
      }
    }
    // En cas d'échec : le SW sera retriggeré par le système (lastChance)
  }
  ```

- [ ] **ATTENTION — Limitation Serwist/Next.js :** Si l'import de `@/lib/local-db` depuis le SW pose des problèmes (directive `"use client"`, imports Next.js), utiliser Dexie directement sans passer par le module :
  ```typescript
  // Alternative si import local-db échoue dans le SW
...
- [ ] Si un toast de confirmation est implémenté lors de la sync background :
  ```json
  "sync": {
...
- [ ] `pendingCount` s'incrémente correctement lors d'une mutation offline (bug fix T1) ✓
- [ ] `registerBackgroundSync()` ne lance pas d'exception si API non supportée ✓
- [ ] Le listener `sync` dans sw.ts est bien câblé ✓
- [ ] Message `TRIGGER_SYNC` déclenche handleTriggerSync dans use-sync-status.ts ✓
...
- [src/lib/sync/outbox.ts] — fichier à MODIFIER (registerBackgroundSync + fix boolean)
- [src/lib/sync/push.ts] — NE PAS MODIFIER — backoff [1000,2000,4000,8000,16000] correct
- [src/hooks/use-sync-status.ts] — fichier à MODIFIER (fix equals(0) + listener SW message)
- [src/app/sw.ts] — fichier à MODIFIER (listener sync event + directSyncFromSW)
- [deferred-work.md §2026-06-24] — Bug `.equals(0)` + guards sync — à corriger dans cette story
...
- `src/app/sw.ts` (à modifier — sync event listener + directSyncFromSW)
- `src/lib/sync/outbox.ts` (à modifier — registerBackgroundSync + fire-and-forget dans applyLocalMutation)
- `src/hooks/use-sync-status.ts` (à modifier — fix boolean equals(0) + SW message listener)
- `src/messages/fr-NE.json` (à modifier optionnellement — clés i18n sync)
- `_bmad-output/implementation-artifacts/sprint-status.yaml` (mis à jour)
- `_bmad-output/implementation-artifacts/6-4-background-sync-api.md` (ce fichier)
